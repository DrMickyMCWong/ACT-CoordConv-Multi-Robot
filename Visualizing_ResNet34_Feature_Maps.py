...
def get_resnet_feature_extractor():
    """Create a ResNet34 feature extractor that returns intermediate features"""
...
def verify_specific_channels(data_dir, channels_to_verify=[1, 131, 95], num_episodes=2, frames_per_episode=3):
    """
    Verify specific channels that are believed to detect particular objects:
    - Channel 1: Robot arm
    - Channel 131: Bowl
    - Channel 95: Brick
    ...
    Args:
        ...
        channels_to_verify: List of channel indices to verify
        num_episodes: Number of episodes to examine
    ...
    # Create figure with one row per channel
    fig_rows = len(channels_to_verify)
    fig_cols = num_episodes * frames_per_episode
    plt.figure(figsize=(5*fig_cols/2, 5*fig_rows))
    ...
    for ep_idx in range(num_episodes):
        ...
        # Get the total number of frames
        with h5py.File(dataset_path, 'r') as root:
            total_frames = len(root['/observations/images/front'])
        
        # Choose frames: beginning, middle, end
        if frames_per_episode == 3:
            frame_indices = [int(total_frames * 0.1), 
                            int(total_frames * 0.5), 
                            int(total_frames * 0.9)]
        ...
        for frame_idx, frame_num in enumerate(frame_indices):
            ...
            # Extract features
            features = extract_features(model, layers, image_tensor)
            feature_tensor = features['layer4'][0].detach().cpu()
            
            # Process each channel
            for ch_idx, channel_idx in enumerate(channels_to_verify):
                # Get activation for this specific channel
                channel_activation = feature_tensor[channel_idx].numpy()
                
                # Resize for visualization
                resized_activation = cv2.resize(channel_activation, (image.shape[1], image.shape[0]))
                ...
                # Plot location: rows are channels, columns are frames
                plot_idx = ch_idx * (num_episodes * frames_per_episode) + ep_idx * frames_per_episode + frame_idx + 1
                plt.subplot(fig_rows, num_episodes * frames_per_episode, plot_idx)
                
                # Plot the overlay
                plt.imshow(overlay.astype(np.uint8))
                
                # Add informative title
                channel_labels = {1: "Arm", 131: "Bowl", 95: "Brick"}
                label = channel_labels.get(channel_idx, f"Ch{channel_idx}")
                plt.title(f"{label}: Ep{ep_idx+1}, Fr{frame_num}")
                plt.axis('off')
    
    plt.tight_layout()
    plt.savefig('channel_verification.png', dpi=300)
    plt.show()

    print("Channel verification complete. Check the visualization to confirm if:")
    print("- Channel 1 consistently activates on the robot arm")
    print("- Channel 131 consistently activates on the bowl")
    print("- Channel 95 consistently activates on the brick")
...
def main():
    ...
    # Verify the specific channels you identified
    print("\nVerifying specific channels (1: arm, 131: bowl, 95: brick)...")
    verify_specific_channels(data_dir, channels_to_verify=[1, 131, 95], num_episodes=2, frames_per_episode=3)
    
    # Optional: Examine each channel individually with more frames for better verification
    for channel, name in [(1, "arm"), (131, "bowl"), (95, "brick")]:
        print(f"\nDetailed verification of channel {channel} ({name})...")
        examine_channel_consistency(channel_idx=channel, num_episodes=2, frames_per_episode=4)
    '''
    ...
    # Check consistency of channel 1 (end effector detector)
    examine_channel_consistency(channel_idx=1, num_episodes=5, frames_per_episode=3)
