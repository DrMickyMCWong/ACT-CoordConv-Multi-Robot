...
import pyrealsense2 as rs  # Add RealSense support
...
def capture_image_realsense(pipeline, align_to_color):
    """
    Method 2: Capture using RealSense pipeline (matches training data)
    This method produces images that match the training data format.
    """
    # Wait for frames
    ...
    # Convert BGR to RGB (same as DemoGen script and training)
    color_image_rgb = cv2.cvtColor(color_image_bgr, cv2.COLOR_BGR2RGB)
    
    # Apply same crop/resize as training (crop top-to-bottom: 0:480, left-to-right: 90:600)
    color_cropped = color_image_rgb[0:480, 90:600]
...
if __name__ == "__main__":
    # Initialize RealSense L515 camera (Method 2 - matches training data)
    print("Initializing Intel RealSense L515 (Method 2)...")
    ...
    if depth_sensor.supports(rs.option.visual_preset):
        ...
        print("✓ Set to Short Range preset")
    
    # Create align object
    align_to_color = rs.align(rs.stream.color)
    
    print(f"✓ RealSense initialized")
    print(f"  Output: {cfg['cam_width']}x{cfg['cam_height']}x3 (RGB)")
    ...
    for i in range(90):
        ...
        _ = capture_image_realsense(pipeline, align_to_color)
    
    obs = {
        'qpos': follower.read_position(),
        'qvel': follower.read_velocity(),
        'images': {cn: capture_image_realsense(pipeline, align_to_color) for cn in cfg['camera_names']}
    }
